...
export async function apiGetBilling(): Promise<BillingData> {
  if (IS_DEVELOPMENT) return MOCK_BILLING;
...
export async function apiGetCurrentSubscription(): Promise<CurrentSubscription | null> {
  ...
  const res = await apiFetch<ApiCurrentSubscriptionResponse>(
...
export async function apiCancelPlan(
...
export async function apiGetSubscriptionHistory(
  ...
  if (IS_DEVELOPMENT)
    return { history: [], total: 0, page: 1, limit, pages: 0 };
...
export async function apiRemovePaymentMethod(id: string): Promise<void> {
  if (IS_DEVELOPMENT) return;
  await apiFetch<{ success: boolean; message: string }>(
    `/app/billing/payment-methods/${id}`,
    {
      method: "DELETE",
    },
  );
...
export async function apiGetBillingAddress(): Promise<BillingAddress | null> {
  if (IS_DEVELOPMENT) return null; // no mock — form stays empty in dev
  try {
    const res = await apiFetch<BillingAddressApiResponse>(
...
const MOCK_PLANS: SubscriptionPlan[] = [
  {
    ...
    benefits: [
      ...
      {
        id: 2,
        name: "2 free telemedicine sessions/month",
        description: "Bronze Plan",
        usageLimit: 0,
        resetFrequency: "never",
      },
      {
        id: 3,
        name: "5% off meals on Mainlandmeals",
        description: "Bronze Plan",
        usageLimit: 0,
        resetFrequency: "never",
      },
      ...
      {
        id: 5,
        name: "Early access to event spaces",
        description: "Bronze Plan",
        usageLimit: 0,
        resetFrequency: "never",
      },
  ...
  },
  {
    ...
    benefits: [
      ...
      {
        id: 7,
        name: "5 telemedicine sessions/month",
        description: "Silver Plan",
        usageLimit: 0,
        resetFrequency: "never",
      },
      ...
      {
        id: 10,
        name: "Event & studio priority booking",
        description: "Silver Plan",
        usageLimit: 0,
        resetFrequency: "never",
      },
  ...
  },
  {
    ...
    benefits: [
      ...
      {
        id: 14,
        name: "15% event & studio discounts",
        description: "Gold Plan",
        usageLimit: 0,
        resetFrequency: "never",
      },
      {
        id: 15,
        name: "10% solar installation discount",
        description: "Gold Plan",
        usageLimit: 0,
        resetFrequency: "never",
      },
...
export async function apiGetPlans(): Promise<SubscriptionPlan[]> {
  if (IS_DEVELOPMENT) return MOCK_PLANS;
...
export async function apiInitiateSubscription(
  payload: SubscribePayload,
): Promise<SubscribeInitResponse> {
  if (IS_DEVELOPMENT) {
    const ref = `LAGOS-${payload.tier.toUpperCase()}-${Date.now().toString().slice(-6)}`;
    if (payload.paymentMethod === "card") {
      return {
        reference: ref,
        authorizationUrl: `/subscribe/confirmed?tier=${payload.tier}&reference=${ref}`,
      };
    }
    return {
      reference: ref,
  ...
  const res = await apiFetch<SubscribeApiResponse>(
    "/app/subscription/initiate",
    {
      method: "POST",
...
export async function apiVerifySubscription(reference: string): Promise<void> {
  if (IS_DEVELOPMENT) return;
  await apiFetch<{ success: boolean; message: string }>(
    `/app/subscription/verify/${reference}`,
  );
}
...
export async function apiValidateCoupon(
  ...
  const res = await apiFetch<CouponApiResponse>(
...
export async function apiInitiateServicePayment(
  ...
  const res = await apiFetch<ServicePaymentApiResponse>(
    "/app/orders/initiate",
    {
      method: "POST",
      body: JSON.stringify(payload),
...
export async function apiGetSubscriptionById(
  id: string | number,
): Promise<SubscriptionDetail | null> {
  if (IS_DEVELOPMENT) return MOCK_SUBSCRIPTION_DETAIL;
  const res = await apiFetch<ApiSubscriptionDetailResponse>(
    `/subscriptions/${id}`,
  );
